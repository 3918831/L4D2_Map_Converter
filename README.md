...
在原 BSP 的副本上应用版本化视觉预设，使用官方 L4D2 VRAD 重烘焙 HDR 世界与静态模型光照，并检查地图结构及玩法数据的保护范围。**0.8.0** 基线包含 C5M1、C4M3、C7M1、C10M3 四套预设、通用转换、批量检查/构建与可选无注入自动 HDR 反射采样，继续支持独立 Windows 原生工具包。新地图从 [通用转换指南](docs/generic-conversion.zh-CN.md) 开始，旧适配入口保持。内置预设无需目标地图 BSP/LMP；无需 VMF、反编译、VBSP 或 VVIS。
...
本版新增通用入口专用的 [`c7m1-hazy-static-v1`](docs/c7m1-preset.zh-CN.md)：C7M1 常态暖光与蓝灰雾，含角色曝光/Bloom、专用太阳材质及有限环境底噪，C2M1/C4M3 输入均已获用户画面验收。新增 [`c10m3-night-v1`](docs/c10m3-preset.zh-CN.md) 提取 C10M3 蓝青色夜间光照、雾与调色，C2M1/C5M1 输入均已获人工接受。预设不移植参考图场景事件。材质残留光泽 VIS-003 按用户要求暂缓，默认仍保留材质。

## 当前范围

0.8.0 增加 [保留来源声音规则 v4](docs/source-audio.zh-CN.md)，四套预设的新配置不再导入目标地图环境音，而是保留原图普通声音、按资源目录剔除天气层。原 0.7.0 规则和接受包仍可复现；新声音规则的机器检查与听音验收单独记录。
...
| `conversion: generic-replace-v3` | 按输入能力检查 → 四套内置预设 | 含已知雷电粒子覆盖；C7M1 目标已有 C2M1/C4M3、C10M3 目标已有 C2M1/C5M1 人工接受；不代表所有组合均已实测 |
...
旧 `source_profile: c6m1_riverbank` 默认 **覆盖原暴雨、雷声、风暴/闪电曝光、局部雾与后处理、检查点调色、雨声环境音**。建筑、物件、局部灯光/材质、碰撞、NAV 和无关推进事件保留；例如新娘 Witch 仍触发尸潮，只取消风暴分支。设置 `"atmosphere_policy": "preserve"` 可使用旧行为。当前是受限 C6 适配，并非任意地图识别器；结构审计通过也不等于完整战役流程通过。
...
| [预设与扩展边界](docs/presets.md) | 四套内置预设、schema 兼容、版本身份和来源适配边界 |
