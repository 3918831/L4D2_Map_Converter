...
未发布版本新增通用入口专用的 [`c7m1-hazy-static-v1`](docs/c7m1-preset.zh-CN.md)：C7M1 常态暖光与蓝灰雾，含角色曝光/Bloom、专用太阳材质及有限环境底噪。C2M1 输入已获用户画面验收，C4M3 输入已完成机器流程、待人工验收；不移植坦克、火灾或局部场景事件。此前 C5M1→C4M3 新输入已获用户接受，材质残留光泽 VIS-003 按用户要求暂缓，默认仍保留材质。
