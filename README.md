...
首个实际批次中，C7M1→C4M3 用户反馈无异常；C4M3→C5M1 在实际观察范围内带无声闪电残留 VIS-002、局部湿润光泽 VIS-003 接受。后续按用户要求新增显式 `generic-replace-v3`，通过资源目录移除已知闪电粒子及直接控制输出，C4M3 新包待独立复测；VIS-003 继续暂缓。详见 [验证与观察记录](docs/validation.md)，不据此宣称完整天气覆盖或其他地图通用验收。
