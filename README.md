...
在原 BSP 的副本上应用版本化视觉预设，使用官方 L4D2 VRAD 重烘焙 HDR 世界与静态模型光照，并检查地图结构及玩法数据的保护范围。**0.7.0** 基线包含 C5M1、C4M3、C7M1、C10M3 四套预设、通用转换、批量检查/构建与可选无注入自动 HDR 反射采样，继续支持独立 Windows 原生工具包。新地图从 [通用转换指南](docs/generic-conversion.zh-CN.md) 开始，旧适配入口保持。内置预设无需目标地图 BSP/LMP；无需 VMF、反编译、VBSP 或 VVIS。
...
当前开发版增加 [保留来源声音规则 v4](docs/source-audio.zh-CN.md)，四套预设的新配置不再导入目标地图环境音，而是保留原图普通声音、按资源目录剔除天气层。原 0.7.0 规则和接受包仍可复现；新声音规则的机器检查与听音验收单独记录。
