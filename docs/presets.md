...
已发布的 0.3.0 内置 `c5m1-daylight-v1`；当前开发版新增实验性 `c4m3-overcast-static-v1`，提取 C4M3 初始化时的固定阴天参数。两者均不需要目标地图 BSP/LMP。C4 首轮游戏采样与原名实机验收待完成，尚未发布新版本；C5 已有接受结果不变。完整游戏资源、官方工具和实际引擎反射采样仍是相应阶段的依赖。
