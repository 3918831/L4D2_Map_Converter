...
当前新配置统一推荐 `generic-replace-v4`：四套目标的视觉参数沿用，普通声音改为保留来源地图并过滤已知天气层。旧预设 JSON 的声音引用仅供旧转换规则复现，v4 不使用这些目标环境音，也不要求其音频依赖。见 [来源声音规则](source-audio.zh-CN.md)。
...
0.7.0 另登记 `c7m1-hazy-static-v1` 与 `c10m3-night-v1`，采用 schema 3、仅开放 `conversion: generic-replace-*` 通用入口，当时使用 v3，当前新建运行推荐 v4；不需要参考地图 BSP 作为每次转换的输入。schema 3 支持空的额外方向光、受限太阳材质路径以及按玩家角色区分的曝光/Bloom，详情见 [C7M1](c7m1-preset.zh-CN.md) 与 [C10M3](c10m3-preset.zh-CN.md) 说明。C7M1 已完成 C2M1/C4M3 输入人工验收，C10M3 已完成 C2M1/C5M1 输入人工验收。
...
预设保存在 `l4d2_bsp/preset_data/<预设ID>.json`，四份 JSON 均在源码 ZIP、wheel 和 sdist 的明确资源清单中。预设 ID、schema/内容版本和文件 SHA256 标识本次使用的数据，运行清单记录并核验这一身份。`v1` 是固定内容版本；改变参数应产生新版本，而不是覆盖旧运行所用文件。C5 保持 schema 1 及原 JSON 字节；C4 使用 schema 2，schema 版本与预设内容版本分别记录。当前加载器只接受已登记内置预设，不提供任意外部 JSON 导入。
