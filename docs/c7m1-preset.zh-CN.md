...
预设 ID：`c7m1-hazy-static-v1`，schema 3。首个原始 C2M1 输入已完成机器流程并获用户画面验收；第二个原始 C4M3 输入已完成烘焙、自动反射采样、最终包审计和测试部署，**等待用户游戏画面验收**。详见 [验证记录](validation.md)。不将已测样本通过扩大为任意地图或全部模式通过，也不称为完整复制 C7M1 场景。
