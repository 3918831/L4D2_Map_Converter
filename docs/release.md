...
源码仓库为 [3918831/L4D2_Map_Converter](https://github.com/3918831/L4D2_Map_Converter)，本版归档标签为 `v0.4.0`。本地版本是否已同步，以远端提交和标签记录为准。项目许可证尚未选择；本版不擅自添加 MIT 或其他许可，也不代表已取得再发行 Valve 内容的授权。公开发布时由项目所有者确定源码许可、第三方说明和最终上传内容。

本机配置、清单和日志可能含个人路径，留在本机。公开问题可附脱敏错误及必要截图；完整 BSP、VPK、NAV、LMP 和官方工具不进入源码仓库。

## 本版状态

0.4.0 提供内置 `c5m1-daylight-v1` 和 `c4m3-overcast-static-v1`，以 `source_profile` 与 `preset` 分离来源适配和目标效果，保留显式 donor 旧配置。C6 晴天最终包已完成 HDR 反射导入，用户测试反馈基本无问题；具体 run、产物哈希和未覆盖范围见 [验证说明](validation.md)。C2→C4 固定阴天已完成 63 份新 HDR 反射导入、八文件包核验及用户原名实机验收，不含新增雨雷或动态风暴。C2→C5 历史接受成果保留为视觉参考，新生成包仍独立验收。旧运行与配置不会自动迁移；更换策略或预设须新配置/输出，单纯归档已接受结果不需要重烘焙。无人值守采样、任意外部预设导入、新增雨区、GUI/EXE、其他地图及 S0—S7 八阶段演示均不是本版发行能力。
