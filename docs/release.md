...
源码仓库为 [3918831/L4D2_Map_Converter](https://github.com/3918831/L4D2_Map_Converter)，0.1.0 初版已上传。后续本地版本是否已同步，以远端提交记录为准。项目许可证尚未选择；本版不擅自添加 MIT 或其他许可，也不代表已取得再发行 Valve 内容的授权。公开发布时由项目所有者确定源码许可、第三方说明和最终上传内容。

本机配置、清单和日志可能含个人路径，留在本机。公开问题可附脱敏错误及必要截图；完整 BSP、VPK、NAV、LMP 和官方工具不进入源码仓库。

## 本版状态

C2 历史接受成果作为视觉参考；新管线生成的包仍独立验收。0.2.0 保留 C6 模型缓存兼容修复，新增默认晴天天气覆盖、局部氛围/环境音替换及采样重载交接。旧运行不会自动迁移，新天气须新配置/输出重新构建与采样。具体离线和实机证据见验证说明。无人值守采样、独立目标预设、新增雨区、GUI/EXE、其他地图及 S0—S7 八阶段演示均不是本版发行能力。
