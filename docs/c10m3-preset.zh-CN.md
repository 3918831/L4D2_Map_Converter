...
预设 ID `c10m3-night-v1`，schema 3，使用通用 `generic-replace-v3` 入口。提取参考图 `c10m3_ranchhouse` 的常态夜间环境；原始 C2M1 输入已获用户画面验收，第二个原始 C5M1 输入已完成烘焙、自动反射采样、最终审计和部署，等待人工复核。详情见 [验证记录](validation.md)，不能将机器流程完成当作实机验收，也不扩大为任意地图通过。
