...
这不是任意脚本的语义分析器。原关卡 BeginScript 调用和加密 .nuc 保留；没有证明外部脚本、addon 或特殊运行模式绝不重新改变天气。因此 replace 的静态结果和完整路线实机结论分别报告。增加其他地图需要新的来源适配；目标独立预设文件与新增天气体积尚未实现。
...
任意地图自动角色识别、自定义地图、独立目标预设、新增天气体积、无引擎等效反射渲染、全部材质依赖验证、无人值守游戏启动、GUI/EXE 和 S0—S7 阶段演示均未实现。静态保护证据不包含完整游戏流程、所有画质/模式或联机兼容验收；C6 replace 需验证无旧天气、Witch 尸潮及其他事件正常，preserve 则仍需观察动态天气对捕获的影响。
